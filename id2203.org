...
* STRT Lecture 11 - Time-leased leader election algorithm
Will not work in partially synchronous system, need stronger model
** Time based leader lease
- Can be integrated with Paxos messages:
  + *As before* acceptor q join round n by sending a Promise in response tona Prepare(n), and promises to not accept proposals in lower rounds
...
*** Clock Drift
Clock drift is the accumulated effect of a clock rate that differs from real time. ideally, dC/dt =1
**** clock drift at proposer
Without compensation,
- Clock runs faster than real time: Safety *cannot be violated* as proposer believes that its lease expired sonner than it acrually did
...
Compensate method:
- dC = dt * (1 - \p) <= whatever seconds * (1 - \p)
**** clock drift at acceptor
without compensation:
...
Compensate method:
- dC = dt * (1+p) >= whatever seconds(e.g. 10) * (1+p)
* STRT Lecture 12 - Shared Memory using Clocks (Interval Clock)
** Synchronized Clocks
** Interval Clocks
- An interval clock (IC) at process p_i read at time t returns a pait C_i(t) = (lo, hi)
- The interval values of a set of clocks read at the same time t are guaranteed to overlap in the correct time
- C_i(t1).lo < C_j(t2).hi
- In write operation, wait until ts(o).t < C_i.lo
* TODO Lecture 13 - Consistent Snapshots
